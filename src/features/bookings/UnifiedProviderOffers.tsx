...
export function UnifiedProviderOffers({ roleLabel = "professional" }: { roleLabel?: string }) {
  ...
  return <div className="space-y-4">
    ...
    <Section title={`New ${roleLabel} offers`} count={pending.length}><div className="space-y-3">{pending.length ? pending.map((offer: any) => { const booking: any = byId.get(offer.booking_id); return booking ? <Card key={offer.id} accent={booking.priority !== "normal"}><div className="flex items-start justify-between gap-3"><div><p className={`text-[11px] font-extrabold uppercase ${booking.priority === "emergency" ? "text-destructive" : "text-clinical-strong"}`}>{booking.priority} · {booking.visit_mode.replace("_", " ")}</p><h3 className="mt-1 text-sm font-bold">{booking.title}</h3><p className="mt-1 text-xs text-muted-foreground">{booking.area ?? booking.city} · {booking.duration_minutes ?? 60} min</p>{booking.service_code ? <p className="mt-1 text-xs font-semibold">Required: {booking.service_code}</p> : null}</div><div className="text-right"><p className="font-bold text-clinical-strong">{offer.earnings ? `₹${offer.earnings}` : "Fee to confirm"}</p>{offer.distance_km != null ? <p className="text-xs text-muted-foreground">{Number(offer.distance_km).toFixed(1)} km</p> : null}</div></div>{booking.description ? <p className="mt-3 text-xs text-muted-foreground">{booking.description}</p> : null}<div className="mt-3 flex items-center gap-2 border-t border-border pt-3"><Button size="sm" onClick={() => accept.mutate(offer.id)} disabled={accept.isPending}><CheckCircle2/>Accept</Button><Button size="sm" variant="outline" onClick={() => decline.mutate(offer.id)} disabled={decline.isPending}><XCircle/>Decline</Button><span className="ml-auto text-[11px] text-muted-foreground"><Clock3 className="mr-1 inline size-3"/>until {new Date(offer.expires_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span></div></Card> : null; }) : <Empty>No new offers within your selected area.</Empty>}</div></Section>
