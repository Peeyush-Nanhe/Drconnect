...
export function UnifiedProviderOffers({ roleLabel = "professional" }: { roleLabel?: string }) {
  ...
  return <div className="space-y-4">
    <div className="flex flex-wrap items-center gap-2">
      <ShareLocationToggle active={active.length > 0} />
      <PhoneAlertsButton />
      <Button asChild size="sm" variant="outline" className="min-h-9">
        <Link to="/matching">Live matching dashboard</Link>
      </Button>
    </div>
    ...
    <div className="grid grid-cols-2 gap-2">
      <Stat label="New offers" value={pending.length}/>
      <Stat label="Active" value={active.length}/>
      <Stat label="Reliability" value={`${hubData?.reliability.score ?? 80}/100`} tone="slate"/>
      <Stat label="Patient rating" value={(() => { const mine=(hubData?.reviews ?? []).filter((review: any) => review.reviewee_id && bookings.some((booking: any) => booking.assigned_provider_id === review.reviewee_id)); return mine.length ? `${(mine.reduce((sum: number, review: any) => sum + review.overall_rating, 0) / mine.length).toFixed(1)}★` : "New"; })()} tone="slate"/>
    </div>
