...
//#region src/lib/homecare-admin.functions.ts?tss-serverfn-split
/** Admins and technicians may monitor the home-care board. */
async function assertMonitor(ctx) {
	...
	const isTechnician = (tech?.data ?? []).length > 0;
...
*/
var getHomeCareBoard_createServerFn_handler = createServerRpc({
	id: "1962e4c6b9fcbdda78d80acb09be63968fa7628913fc8aa90a2be99d11be7c67",
	name: "getHomeCareBoard",
	filename: "src/lib/homecare-admin.functions.ts"
}, (opts) => getHomeCareBoard.__executeServer(opts));
var getHomeCareBoard = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((d) => ({ days: Math.min(Math.max(Number(d?.days ?? 30), 1), 365) })).handler(getHomeCareBoard_createServerFn_handler, async ({ data, context }) => {
	const scope = await assertMonitor(context);
	const sb = await adminClient();
	const since = (/* @__PURE__ */ new Date(Date.now() - data.days * 24 * 60 * 60 * 1e3)).toISOString();
	const [{ data: visits }, { data: therapists }, { data: partners }, { data: careRequests }, { data: plans }, { data: availability }, { data: profiles }] = await Promise.all([
		sb.from("physio_visits").select("id, area, city, therapy_type, status, urgency, scheduled_at, session_number, duration_min, fee, therapist_id, partner_id, patient_name, checked_in_at, checked_out_at, no_show, created_at").gte("created_at", since).order("scheduled_at", { ascending: true }),
		sb.from("physio_therapists").select("id, full_name, city, area, specializations, active, verified, partner_id, user_id, phone"),
		sb.from("physio_partners").select("id, name"),
		sb.from("care_requests").select("id, specialty, status, scheduled_for, my_doctor_id, preferred_id, accepted_by, patient_id, created_at, completed_at, amount").gte("created_at", since).order("created_at", { ascending: false }),
		sb.from("family_physician_plans").select("id, doctor_name, doctor_spec, status, price, purchased_at, expires_at, patient_id"),
	...
	const nameOf = (id) => id && (profiles ?? []).find((p) => p.id === id)?.full_name || null;
	const partnerName = (id) => id && (partners ?? []).find((p) => p.id === id)?.name || null;
	const physioVisits = (visits ?? []).map((v) => ({
		id: v.id,
		area: v.area,
		city: v.city,
		...
		therapistName: (therapists ?? []).find((t) => t.id === v.therapist_id)?.full_name ?? null,
	...
	const therapistRows = (therapists ?? []).map((t) => {
		...
		const availRow = (availability ?? []).find((a) => a.user_id === t.user_id);
		return {
			id: t.id,
			name: t.full_name,
			city: t.city,
	...
	const doctorBookings = (careRequests ?? []).map((r) => {
	...
	const doctorRoster = Array.from(new Set((careRequests ?? []).map((r) => r.accepted_by ?? r.my_doctor_id ?? r.preferred_id).filter(Boolean))).map((id) => {
		const availRow = (availability ?? []).find((a) => a.user_id === id);
	...
	return {
		...
		totals: {
			...
			homeCarePlans: (plans ?? []).filter((p) => p.status === "active").length
		},
		therapyMix,
		areaMix,
		...
		doctorRoster,
		homeCarePlans: (plans ?? []).slice(0, 40).map((p) => ({
			id: p.id,
...
//#endregion
export { getHomeCareBoard_createServerFn_handler };
