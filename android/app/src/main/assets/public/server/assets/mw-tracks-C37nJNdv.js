//#region src/features/mydox/mw-tracks.ts
var inr = (n) => typeof n === "number" ? "₹" + n.toLocaleString("en-IN") : "—";
var ROLE = {
	psychiatrist: {
		role: "psychiatrist",
		label: "Psychiatrist",
		desc: "Diagnosis, medication, risk review"
	},
	psychologist: {
		role: "clinical_psychologist",
		label: "Clinical Psychologist",
		desc: "CBT and talking therapy (RCI registered)"
	},
	coordinator: {
		role: "care_coordinator",
		label: "Care Coordinator",
		desc: "Your named contact — arranges the whole team"
	},
	physician: {
		role: "physician",
		label: "Physician",
		desc: "Rules out thyroid, B12, vitamin D, anaemia"
	},
	cardiologist: {
		role: "cardiologist",
		label: "Cardiologist",
		desc: "Rules out cardiac causes of palpitations and chest pain"
	},
	occupational: {
		role: "occupational_therapist",
		label: "Occupational Therapist",
		desc: "Daily activity, routine, return to work"
	},
	dietitian: {
		role: "dietitian",
		label: "Dietitian",
		desc: "Appetite, weight and metabolic support"
	},
	socialworker: {
		role: "psychiatric_social_worker",
		label: "Psychiatric Social Worker",
		desc: "Family support, benefits, community linkage"
	},
	nurse: {
		role: "psychiatric_nurse",
		label: "Psychiatric Nurse",
		desc: "Medication support and monitoring"
	},
	vocational: {
		role: "vocational_rehab",
		label: "Vocational Rehabilitation",
		desc: "Skills, job readiness, supported employment"
	},
	urologist: {
		role: "urologist",
		label: "Urologist",
		desc: "Physical assessment and treatment"
	},
	endocrinologist: {
		role: "endocrinologist",
		label: "Endocrinologist",
		desc: "Hormone and diabetes assessment"
	},
	dermatologist: {
		role: "dermatologist",
		label: "Dermatologist",
		desc: "Skin and related conditions"
	},
	gynaecologist: {
		role: "gynaecologist",
		label: "Gynaecologist",
		desc: "Women's health assessment"
	},
	pelvicphysio: {
		role: "pelvic_physiotherapist",
		label: "Pelvic Floor Physiotherapist",
		desc: "Pelvic floor assessment and therapy"
	},
	couples: {
		role: "couples_therapist",
		label: "Couples Therapy",
		desc: "Sessions together with your partner"
	}
};
var req = (r) => ({
	...r,
...
function scoreBand(instrument, score) {
	if (instrument === "GAD-7") {
		if (score <= 4) return {
			band: "Minimal",
			tone: "#16A34A"
...
function modalityFor(track, score) {
	if (track.modality === "in_person") return {
		mode: "in_person",
		reason: track.modalityNote
	};
	if (track.modality === "conditional" && score >= (track.inPersonAt ?? 15)) return {
		mode: "in_person",
		reason: track.modalityNote
	};
	return {
...
var MW_TRACKS = [
	{
		...
		modalityNote: null,
		team: [
			req(ROLE.psychiatrist),
			req(ROLE.psychologist),
			req(ROLE.coordinator),
			opt(ROLE.physician),
			opt(ROLE.occupational),
			opt(ROLE.dietitian)
		],
	...
	},
	{
		...
		team: [
			...
			req(ROLE.coordinator),
			opt(ROLE.urologist),
			opt(ROLE.gynaecologist),
			opt(ROLE.endocrinologist),
			opt(ROLE.dermatologist),
			opt(ROLE.pelvicphysio),
			opt(ROLE.couples)
		],
...
var URGENT_CONTACTS = [{
	label: "Tele-MANAS — national mental health helpline",
	value: "14416",
	note: "Free, 24×7, multiple languages"
}, {
	label: "KNCH emergency",
	value: "90-911-911-90",
	note: "Wakad, Pune"
}];
//#endregion
export { inr as a, URGENT_CONTACTS as i, RESPONSES as n, modalityFor as o, SLA as r, scoreBand as s, MW_TRACKS as t };
