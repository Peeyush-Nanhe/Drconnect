...
//#region src/lib/health-companion.functions.ts?tss-serverfn-split
var SYSTEM = (todayISO, todayLabel) => `You are MyDox's AI Health Companion — a warm, friendly health buddy the patient chats with in text and voice to share symptoms, daily notes, medication updates, and follow-ups.
...
var chatHealthCompanion_createServerFn_handler = createServerRpc({
	id: "f8870202e4df4809d36d0c82b588b86647abec6ba6fe82617c867cb0f9090231",
	name: "chatHealthCompanion",
	filename: "src/lib/health-companion.functions.ts"
}, (opts) => chatHealthCompanion.__executeServer(opts));
var chatHealthCompanion = createServerFn({ method: "POST" }).inputValidator((d) => {
	if (!d || !Array.isArray(d.messages)) throw new Error("messages required");
	return { messages: d.messages.filter((m) => m && (m.role === "user" || m.role === "assistant") && typeof m.content === "string").map((m) => ({
		role: m.role,
		content: m.content.slice(0, 8e3)
	})).slice(-40) };
}).handler(chatHealthCompanion_createServerFn_handler, async ({ data }) => {
...
//#endregion
export { chatHealthCompanion_createServerFn_handler };
